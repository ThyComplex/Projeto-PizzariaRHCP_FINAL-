...
/* This is the Total_value update procedure, it searches for each item on the receit and calculates how much it has to be paid */
use bd_pizzaria;
--  drop procedure gettotalvalue;

DELIMITER // 
create procedure gettotalvalue()
	begin
set @maxv := (select max(nu_pedido) from tb_pedido);
set @minv := (select min(nu_pedido) from tb_pedido);    
   l1: loop 
		if @minv > @maxv
			then 
				 leave l1;
...
update tb_pedido
set tb_pedido.vl_total = (select distinct sum(tb_pedido_itens.qt_itens*tb_itens.vl_valor_unitario) as VL 
	from tb_pedido
    inner join tb_pedido_itens on tb_pedido_itens.nu_pedido = tb_pedido.nu_pedido 
    inner join tb_itens on tb_itens.cd_itens = tb_pedido_itens.cd_itens 
    where tb_pedido.nu_pedido = @minv)where tb_pedido.nu_pedido = @minv;
    set @minv := @minv + 1; 
